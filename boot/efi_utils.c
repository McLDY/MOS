...
EFI_STATUS boot_kernel(void) {    
    ...
    for (UINTN i = 0; i < max_mode; i++) {
        ...
        // 选择最佳模式（优先1024x768及以上）
        if (mode_info->HorizontalResolution >= 1024 && mode_info->VerticalResolution >= 768) {
            if (mode_info->HorizontalResolution > best_width || 
                (mode_info->HorizontalResolution == best_width && mode_info->VerticalResolution > best_height)) {
                best_mode = i;
    ...
    if (best_width == 0) {
        ...
        print_string(L"[->] Choose the best mode: ");
